...
--- @vararg any Alternating labels and values
function LIB.PrintPairs( ... )
    if not LIB.IsEnabled then return end
    if LIB.HasHitMaxDepth() then return end
    if LIB.SectionLabelFilter then
        if #LIB.SectionStack == 0 then
    ...
    local args = { ... }
    local privatePrintArgs = {}
    for labelIndex = 1, #args, 2 do
        local label = args[labelIndex]
        local value = args[labelIndex + 1]
        local valueToPrint = ( value ~= nil and value or "nil" )

        privatePrintArgs[#privatePrintArgs + 1] = label
        privatePrintArgs[#privatePrintArgs + 1] = ": "
        privatePrintArgs[#privatePrintArgs + 1] = valueToPrint

        if labelIndex ~= #args - 1 then
            privatePrintArgs[#privatePrintArgs + 1] = ", "
        end
    end

    LIB.PrivatePrint( unpack( privatePrintArgs ) )
end

--- @param condition boolean|fun():boolean
--- @vararg ...
function LIB.PrintWhen( condition, ... )
