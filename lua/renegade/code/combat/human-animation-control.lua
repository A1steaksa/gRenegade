...
function INSTANCE:Update( deltaTime )
    ...
    if self.Model ~= nil then
        ...
        elseif totalAnimations == 1 then
            ...
            local overrideFrame = 0

            if toolsLib.IsToolOpen( "ren_animation_tool" ) then
                local frame = toolsLib.GetToolFrame( "ren_animation_tool" )

                if IsValid( frame ) then
                    local state = frame:ExportState() --[[@as AnimationToolState]]

                    if not state.IsPlaying then
                        shouldOverride = true
                        overrideFrame = state.FrameNumber
                    end
            ...
            if shouldOverride then
                self.Model:SetAnimation( self.DataList[1].Animation, overrideFrame )
            else
