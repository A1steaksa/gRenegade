...
function PrintByteStringHex( label, byteString )
    local byteStrings = {}
    for i = 1, string.len( byteString ) do
        local rawByteString = byteString:sub( i, i )
        local byteString = string.byte( rawByteString )
        local byteNumber = tonumber( byteString )
    ...
    print( labelText .. table.concat( byteStrings, " " ) )
end

--- @param label string
--- @param byteString string
function PrintByteStringBinary( label, byteString )
    ...
    for i = 1, #byteString do
        ...
        for j = 0, 7 do
            ...
            if j == 3 then
                byteBinaryString = " " .. byteBinaryString
            end

            char = math.floor( char / 2 )
        end

        outputBinaryString = byteBinaryString .. " " .. outputBinaryString
    end
